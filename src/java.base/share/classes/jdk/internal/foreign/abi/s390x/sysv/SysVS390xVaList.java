...
public non-sealed class SysVS390xVaList implements VaList, Scoped {
	...
	static final GroupLayout LAYOUT_VA_LIST = MemoryLayout.structLayout(
	...
	private static final MemoryLayout UNUSED_AREA = MemoryLayout.paddingLayout(128)
											.withBitAlignment(64).withName("UnusedArea");

	/* The other register save area is 72 bytes in size. */
	private static final MemoryLayout OTHER_REG_SAVE_AREA = MemoryLayout.paddingLayout(576)
	...
	private final MemorySegment overflowArgArea;
	...
	private SysVS390xVaList(MemorySegment segment, MemorySegment gpRegSaveArea, MemorySegment fpRegSaveArea, MemorySegment overflowArgArea) {
		...
		this.regSaveArea = MemorySegment.ofAddress((MemoryAddress)VH_REG_SAVE_AREA.get(segment),
													LAYOUT_REG_SAVE_AREA.byteSize(), segment.session());
		this.overflowArgArea = getInitialOverFlowArgArea(segment, overflowArgArea);
		this.overflowAreaCursor = overflowArgArea;
	}

	private MemorySegment getInitialOverFlowArgArea(MemorySegment vaListSegment, MemorySegment overflowArgArea) {
		MemorySegment initOverflowArgArea = null;

		/* The addresses of the overflow area of va_list and the passed-in overflow area should be
		 * identical when the passed-in overflow area holds the initial address of the overflow area;
		 * otherwise, the passed-in overflow area represents the location at the latest offset in the
		 * overflow area after invoking skip()/nextVarg() to move forwards.
		 */
		MemorySegment overflowArgAreaOfVaList = MemorySegment.ofAddress((MemoryAddress)VH_OVERFLOW_ARG_AREA.get(vaListSegment),
																			Long.MAX_VALUE, vaListSegment.session());
		if (overflowArgAreaOfVaList.address().toRawLongValue() == overflowArgArea.address().toRawLongValue()) {
			/* Set the overflow area with the passed-in overflow area which still
			 * holds the exact size of the area.
			 */
			initOverflowArgArea = overflowArgArea;
		} else {
			/* Set the overflow area with the value of __overflow_arg_area in va_list
			 * given the passed-in overflow area points to the latest offset rather
			 * than the initial address of the overflow area.
			 */
			initOverflowArgArea = overflowArgAreaOfVaList;
		}

		return initOverflowArgArea;
	}
	 ...
	 */
	private static boolean isRegOverflow(boolean isFloatOrDouble, long usedGprNum, long usedFprNum) {
		return ((!isFloatOrDouble && (usedGprNum > MAX_GPR_NUM))
				|| (isFloatOrDouble && (usedFprNum > MAX_FPR_NUM)));
	}
	...
	private void checkRegSaveAreaElement(MemoryLayout layout) {
		boolean isFloatOrDouble = TypeClass.isFloatingType(layout);
		if ((!isFloatOrDouble && (gpRegSaveArea.byteSize() < VA_LIST_SLOT_BYTES))
			|| (isFloatOrDouble && (fpRegSaveArea.byteSize()< VA_LIST_SLOT_BYTES))
		) {
			throw SharedUtils.newVaListNSEE(layout);
		}
	}
	...
	private Object getArgFromMemoryArea(MemoryLayout layout, MemorySegment argAreaSegment, SegmentAllocator allocator, boolean isOverflowArea) {
		...
		switch (typeClass) {
			case STRUCT_ONE_FLOAT -> {
				...
				MemorySegment struArgSegment = allocator.allocate(struArgSize);
	...
	public static VaList ofAddress(MemoryAddress addr, MemorySession session) {
		...
		MemorySegment regSaveArea = MemorySegment.ofAddress((MemoryAddress)VH_REG_SAVE_AREA.get(segment),
															LAYOUT_REG_SAVE_AREA.byteSize(), session);
		MemorySegment overflowArgArea = MemorySegment.ofAddress((MemoryAddress)VH_OVERFLOW_ARG_AREA.get(segment),
		 ...
		 */
		MemorySegment gpRegSaveArea = regSaveArea.asSlice(GPR_OFFSET + initGprNo * VA_LIST_SLOT_BYTES,
															(MAX_GPR_NUM  - initGprNo) * VA_LIST_SLOT_BYTES);
		MemorySegment fpRegSaveArea = regSaveArea.asSlice(FPR_OFFSET + initFprNo * VA_LIST_SLOT_BYTES,
															(MAX_FPR_NUM - initFprNo) * VA_LIST_SLOT_BYTES);

		return new SysVS390xVaList(segment, gpRegSaveArea, fpRegSaveArea, overflowArgArea);
	}
	...
	@Override
	public VaList copy() {
		MemorySegment copySegment = MemorySegment.allocateNative(LAYOUT_VA_LIST, segment.session());
		return new SysVS390xVaList(copySegment.copyFrom(segment), gpRegSaveArea, fpRegSaveArea, overflowAreaCursor);
	}
	...
	public String toString() {
		return "SysVS390xVaList{"
				+ "__gpr=" + currentGprNo()
				+ ", __fpr=" + currentFprNo()
				+ ", __overflow_arg_area=" + overflowArgArea
				+ ", __regSaveArea=" + regSaveArea
				+ '}';
	}
			...
			for (SimpleVaArg arg : vaListArgs) {
				...
				switch (typeClass) {
					case STRUCT_ONE_FLOAT, STRUCT -> {
						...
						long struArgSize = struArgValue.byteSize();
						...
						switch (typeClass) {
							case STRUCT_ONE_FLOAT -> {
								 ...
								 */
								long rightShiftBytes = isOverflowArea ? (VA_LIST_SLOT_BYTES - struArgSize) : 0;
								argAreaCursor.asSlice(rightShiftBytes, struArgSize).copyFrom(struArgValue);
							}
							case STRUCT -> {
